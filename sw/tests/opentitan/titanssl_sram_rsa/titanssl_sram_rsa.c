...
#include "sw/device/lib/dif/difbn.h"
...
void titanssl_benchmark_memcpy32_aligned_frombn(
        mmio_region_t otbn,
        size_t offset,
        uint8_t* dst,
        size_t n)
...
__attribute__((always_inline))
void titanssl_benchmark_memcpy32_aligned_tobn(
        mmio_region_t otbn,
        size_t offset,
        const uint8_t* src,
        size_t n)
{
    ...
    printf("=== titanssl_benchmark_memcpy32_aligned_tobn ===\r\n");
    printf("offset: %d\r\n", offset);
    printf("src: 0x%08x\r\n", src);
    printf("n: %d\r\n", n);
        ...
        titanssl_buffer_t *const modulus)
{
    ...
    titanssl_benchmark_memcpy32_aligned_tobn(
        otbn,
    ...
    // Read back results.
    titanssl_benchmark_memcpy32_aligned_frombn(
        otbn,
        OTBN_DMEM_REG_OFFSET+kOtbnVarRsaInOut, 
        cipher->data, 
        TITANSSL_SIZE_KEY
        ...
        titanssl_buffer_t *const private)
{
    ...
    mode = 2;
    otbn_copy_data_tobn(&otbn_ctx, sizeof(uint32_t), &mode, kOtbnVarRsaMode);
    otbn_copy_data_tobn(&otbn_ctx, sizeof(uint32_t), &n_limbs, kOtbnVarRsaNLimbs);
    otbn_copy_data_tobn(&otbn_ctx, TITANSSL_SIZE_KEY, modulus->data, kOtbnVarRsaModulus);
    otbn_copy_data_tobn(&otbn_ctx, TITANSSL_SIZE_KEY, private->data, kOtbnVarRsaExp);
    otbn_copy_data_tobn(&otbn_ctx, TITANSSL_SIZE_KEY, plain->data, kOtbnVarRsaInOut);
    ...
    // Read back results.
    otbn_copy_data_frombn(&otbn_ctx, TITANSSL_SIZE_KEY, kOtbnVarRsaInOut, cipher->data);
}
