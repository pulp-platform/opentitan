...
//
//
#include "../flash_carfield_boot/bazel-out/flash_carfield_boot_signed32.h"
...
int main(int argc, char **argv) {
  ...
  *debug_mode = 0x00000001;
  ...
  for(int i = 0; i < buffer_size; i += 3) {
     if(i + 2 < buffer_size) {
        *payload_1 = FLASH_CARFIELD_BOOT_SIGNED[i];
        *payload_2 = FLASH_CARFIELD_BOOT_SIGNED[i+1];
        *payload_3 = FLASH_CARFIELD_BOOT_SIGNED[i+2];
        *address = i/3;
        *start = 0x1;
     }
  }
