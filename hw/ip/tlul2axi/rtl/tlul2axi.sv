  ...
  #(
   ...
   always_comb  begin
    ...
    case(state_q)
      ...
      WAIT_R_VALID: begin
        if(axi_req_i.r_valid && axi_req_o.r_ready) 
          state_d = IDLE;
        else
          state_d = WAIT_R_VALID;
      end
      ...
      WAIT_B_VALID: begin
        if(axi_req_i.b_valid && axi_req_o.b_ready) 
          state_d = IDLE;
        else
          state_d = WAIT_B_VALID;
      end
   ...
   always_comb begin : axi_mst_output_fsm
    ...
    case (state_q)
      ...
      IDLE: begin
        if(tl_i.a_valid) begin        // request   
          if(tl_i.a_opcode == tlul_pkg::Get) begin // get
            ...
            if(axi_req_i.aw_ready && axi_req_i.w_ready)
              tl_o_int.a_ready = 1'b1;
          end                   
        end 
      end
      ...
      WAIT_B_VALID: begin
        if(axi_req_i.b_valid) begin
          tl_o_int.d_source = axi_req_i.b.id;
          tl_o_int.d_opcode = tlul_pkg::AccessAck;
          tl_o_int.d_error  = axi_req_i.b.resp[1];
      ...
      WAIT_R_VALID: begin
        if(axi_req_i.r_valid) begin
          tl_o_int.d_source = axi_req_i.r.id;
          tl_o_int.d_opcode = tlul_pkg::AccessAckData;
          tl_o_int.d_error  = axi_req_i.r.resp[1];
          tl_o_int.d_data   = axi_req_i.r.data;
          tl_o_int.d_valid  = 1'b1;
